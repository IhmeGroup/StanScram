...
class JICModel:
    ...
    def __init__(
        ...
        if load_chemical_sources:
            L_probe = np.load(datadir / "L_probe.npy")
            omega_C = np.load(datadir / "omega_C.npy")
            self.omega_C_interpolators = []
            for i in range(len(self.x)):
                omega_C_i = interpolate.RegularGridInterpolator(
                    (self.mdot_inj_unique, L_probe), omega_C[i]
                )
    ...
    def get_injector_sources(self, rho, rhoU, E, rhoZ, rhoC, gamma, t):
        ...
        last_fluid_tip = (
            len(self.fluid_tips)
            - np.searchsorted(self.fluid_tips[::-1, 0], self.x, side="right")
            - 1
        )
        mdot_profile = np.zeros_like(self.x)
        mdot_profile[last_fluid_tip > 0] = self.fluid_tips[
            last_fluid_tip[last_fluid_tip > 0], 1
        ]
    ...
    def calc_chemical_sources(self, write=False):
        ...
        if write:
            np.save(datadir / "L_probe.npy", L_probe)
            omega_C = np.zeros((len(self.x), len(self.mdot_inj_unique), len(L_probe)))

        def do_nothing(points: tuple[float, float]):
            return 0.0
        ...
        for i in tqdm(range(len(self.x))):
            ...
            if self.x[i] > self.x_noz:
                ...
                b = a * (1 - Z_avg) / Z_avg
                p_Z = stats.beta.pdf(
                    np.tile(Z_probe, (len(self.mdot_inj_unique), 1)),
                    np.tile(a, (len(Z_probe), 1)).T,
                    np.tile(b, (len(Z_probe), 1)).T,
                )
                omega_C_probe = self.fpv_table.lookup(
                    "SRC_PROG", Z_mesh_ZL, 0.0, L_mesh_ZL
                )  # [1/s]
                ...
                omega_C_i = interpolate.RegularGridInterpolator(
                    (self.mdot_inj_unique, L_probe), omega_C_probe_mdots
                )
                if write:
        ...
        if write:
            np.save(datadir / "omega_C.npy", omega_C)
